...
static void uart_init(void){
    ...
    /* TODO A: 수신 인터럽트 개방 — IMSC의 RXIM(bit4)를 켠다 */
}
...
void irq_handler(void){
    ...
    /* TODO B: 손님 2 — UART(bit12). 원인 제거는 DR "읽기" 자체(24차시 소비 규약).
       읽은 문자는 버려도 좋다(카운트만) — rx++; */
    (void)st;
}
...
int main(void){
    ...
    puts_("IRQ34 START\r\n");
    VIC_INTENABLE = BIT(4);
    /* TODO C: 두 번째 문 — VIC에 UART 소스(bit12)도 개방한다 */
    enable_irq();
